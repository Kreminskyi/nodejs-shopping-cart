...
svgedit.utilities.buildJSPDFCallback = function (callJSPDF) {
	return svgedit.utilities.executeAfterLoads('RGBColor', ['javascripts/svg-edit/canvg/rgbcolor.js'], function () {
		var arr = [];
		if (!RGBColor || RGBColor.ok === undef) { // It's not our RGBColor, so we'll need to load it
			arr.push('javascripts/svg-edit/canvg/rgbcolor.js');
		}
		svgedit.utilities.executeAfterLoads('jsPDF', arr.concat('javascripts/svg-edit/jspdf/underscore-min.js', 'javascripts/svg-edit/jspdf/jspdf.min.js', 'javascripts/svg-edit/jspdf/jspdf.plugin.svgToPdf.js'), callJSPDF)();
	});
};
